...
adam = Adam(lr=0.02, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0)
...
def scheduler(epoch):
  if epoch==5:
    model.lr.set_value(.002)
    return model.lr.get_value()

change_lr = LearningRateScheduler(scheduler)

model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=epoch, batch_size=25, verbose=1, callbacks=['change_lr'])
